...
$qry2 = "SELECT sum(total) as tt from sales where status='pending' and billid='0'";  
...
?>
			...
			<table class="balance">
				<tr>
					...
					<td><span data-prefix>&#8377;</span><span><?php echo $row2['tt']; 
				
					$_SESSION['tota']=$row2['tt']; ?></span></td>
...
if(isset($_REQUEST['print']))
{
            ...
            $i="insert into transaction(NAME,CUSTOMER,DATE,TOT)values('$NAME','$CUSTOMER','$dates','$total')";
            $u="update sales SET status='sold' where status='pending'";
            if($con->query($i) && $con->query($u))
    {
        ?>
        <script>
        window.location="Adminsales.php";
        </script>
        <?php
